...
let cjs = null;
try {
  esm = await import("harumi");
  push("S5a ESM import('harumi') が解決する", true, Object.keys(esm).sort().join(","));
} catch (error) {
  push("S5a ESM import('harumi') が解決する", false, error.message);
}
try {
  cjs = require("harumi");
  push("S5a CJS require('harumi') が解決する", true, Object.keys(cjs).sort().join(","));
} catch (error) {
  push("S5a CJS require('harumi') が解決する", false, error.message);
}
// 解決先が staging（＝配布された tarball の中身）であることを要求する。
// repo の作業ツリーへ解決されていたら S5 は成立していない（Node の self-reference 経路）。
try {
  const resolved = require.resolve("harumi");
...
push(
  "S5a ambiguousSearch が ESM / CJS の双方から取れる",
  typeof esm?.ambiguousSearch === "function" && typeof cjs?.ambiguousSearch === "function",
  "esm=" + typeof esm?.ambiguousSearch + " cjs=" + typeof cjs?.ambiguousSearch,
);

// (b) ambiguousSearch の基本検索・年範囲・空入力
const search = esm?.ambiguousSearch ?? cjs?.ambiguousSearch;
if (typeof search !== "function") {
  push("S5b ambiguousSearch の3ケース", false, "ambiguousSearch を取得できません");
} else {
  try {
    const basic = search("寛永");
    push(
      "S5b 基本検索 '寛永' が結果を返し先頭が 1624 / 寛永1 である",
      Array.isArray(basic) && basic.length > 0 && basic[0].year === 1624 && basic[0].nengo === "寛永1",
      "len=" + (Array.isArray(basic) ? basic.length : "not-array") + " head=" + JSON.stringify(basic?.[0] ?? null),
    );
    const ranged = search("寛永", { range: "1624-1630" });
    push(
      "S5b 年範囲 1624-1630 が範囲内のみを返す",
      Array.isArray(ranged) &&
        ranged.length > 0 &&
        ranged.length < basic.length &&
        ranged.every((entry) => entry.year >= 1624 && entry.year <= 1630),
      "len=" + (Array.isArray(ranged) ? ranged.length : "not-array"),
    );
    const empty = search("");
    push(
      "S5b 空入力が空配列を返す",
      Array.isArray(empty) && empty.length === 0,
      "len=" + (Array.isArray(empty) ? empty.length : "not-array"),
    );
  } catch (error) {
    push("S5b ambiguousSearch の3ケース", false, error.message);
  }
}
